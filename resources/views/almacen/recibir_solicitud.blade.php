...
@section('cuerpo')
    <div style="width: 70%;height:7%;float: left; position: relative">
        <div style="width:25%; float: left; position: relative">
            <input id = "numsol" class="form-control" type="text" placeholder="Solicitud" style="width: 95%">
        </div>
        &nbsp;
        <div style="width: 50%; float: left; position: relative;" >
            <button type="button" id = "buscarsol" class="btn btn-secondary btn-sm" style="height: 38px">Buscar Solicitud</button>
    ...
    <div style="width: 70%; float: left; position: absolute; top: 45%" class="bordes">
        ...
        <p></p>
        <button type="button" id = "recibirsol" class="btn btn-secondary btn-sm" style="visibility: hidden;height: 38px;margin-left: 2px">Recibir Solicitud</button>
        <p></p>
    </div>
...
@section('js_bodega')
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $(document).on('click', '#buscarsol',function () {

                var url ="{{route('bodega.solicitud',':id')}}".replace(':id',$('#numsol').val());
                var url2="{{route('bodega.solicitud_detalle',':id')}}".replace(':id',$('#numsol').val());
            ...
            $(document).on('click', '#recibirsol',function () {
                var data = {"solicitud": $('#numsol').val()};
                $.ajaxSetup({
                    headers: {
                ...
                $.ajax({
                    ...
                    url: '{{route('recibir_almacen.post')}}',
                    method: 'POST',
        ...
        function actualiza_det_oc() {
            ...
            $.ajax({
                ...
                url: '{{route('recibir_almacen_detalle.post')}}',
                method: 'POST',
